...
export class UIScene extends Phaser.Scene {
  ...
  private buildInventoryPanel() {
    ...
    if (isBag) {
      ...
      const els: Array<ElementType> = ['fire', 'electro', 'plasma']
      els.forEach((el, i) => {
      ...
      // Baúl: mostrar coins en banco
      const bancoLbl = addLabel(this, 28, 60,
        `◆ ${GameState.stashCoins} coins en banco`, 12, CSS.yellow).setOrigin(0, 0)
      ammoObjs.push(bancoLbl)
    }

    const rows: Phaser.GameObjects.GameObject[] = []
    const itemsStartY = isBag ? 100 : 86
    if (items.length === 0) {
  ...
  private buildStatsPanel() {
    ...
    if (invested > 0) {
      ...
      const resetLbl = `[RESET — ${cost} ◈ banco]`
  ...
  private setupTouchControls() {
    ...
    // 3 skills en triángulo — bottom right
